...
exports.getNearbyProperties = exports.getTopProperties = exports.getPropertyById = exports.calculateBookingPrice = void 0;
...
const getTopProperties = async (req, res) => {
    try {
        ...
        const mappedProperties = properties.map((prop) => {
            const totalTransactions = prop.roomTypes.reduce((acc, room) => acc +
                room.transactions.filter((t) => t.status === "ACCEPTED").length, 0);
            const minPrice = prop.roomTypes.length
                ? Math.min(...prop.roomTypes.map((r) => r.price))
                : null;
            const availableRooms = prop.roomTypes.reduce((acc, room) => acc +
                (room.quota -
                    room.transactions.filter((t) => t.status === "ACCEPTED").length), 0);
...
const getNearbyProperties = async (req, res) => {
    try {
        ...
        const resultLimit = parseInt(limit) || 6;
        ...
        console.error("Error in getNearbyProperties:", err);
        return res.status(500).json({ message: "Server error" });
    }
};
