...
exports.getPeakSeasonHistory = exports.deletePeakSeason = exports.createPeakSeason = exports.getPeakSeasons = void 0;
const prisma_1 = require("../config/prisma");
const getPeakSeasons = async (req, res) => {
    try {
        const user = req.user;
        if (!user?.id)
        ...
        const roomType = await prisma_1.prisma.roomType.findFirst({
            where: {
                id: roomTypeId,
                property: {
                    userId: user.id
        ...
        const peakSeasons = await prisma_1.prisma.peakSeason.findMany({
            where: { roomTypeId },
...
const createPeakSeason = async (req, res) => {
    try {
        const user = req.user;
        if (!user?.id)
            return res.status(401).json({ error: "Unauthorized" });
        ...
        const overlapping = await prisma_1.prisma.peakSeason.findFirst({
            where: {
        ...
        const peakSeason = await prisma_1.prisma.peakSeason.create({
            data: {
                roomTypeId: Number(roomTypeId),
                startDate: start,
                endDate: end,
...
exports.createPeakSeason = createPeakSeason;
const deletePeakSeason = async (req, res) => {
    try {
        ...
        const peakSeason = await prisma_1.prisma.peakSeason.findFirst({
            where: {
                id: peakSeasonId,
                roomType: {
                    property: {
        ...
        await prisma_1.prisma.peakSeason.delete({
...
exports.deletePeakSeason = deletePeakSeason;
const getPeakSeasonHistory = async (req, res) => {
    try {
        const user = req.user;
        if (!user?.id)
            return res.status(401).json({ error: "Unauthorized" });
        ...
        const history = await prisma_1.prisma.peakSeason.findMany({
            where: { roomTypeId },
...
exports.getPeakSeasonHistory = getPeakSeasonHistory;
