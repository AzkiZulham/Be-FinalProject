...
export const getPropertyCatalog = async (req: Request, res: Response) => {
  try {
    const {
      search = "",
      category = "",
    ...
    const formattedProperties = properties.map(property => {
      const transactionCount = sortBy === "transactionCount" ? property.roomTypes.reduce((sum, rt) => sum + (rt as any).transactions.length, 0) : undefined;
      return {
        id: property.id,
        name: property.name,
